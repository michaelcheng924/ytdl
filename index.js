...
const audio = ytdl(ref, { quality: "lowestaudio" }).on(
  "progress",
  (_, downloaded, total) => {
    tracker.audio = { downloaded, total };
  }
);
const video = ytdl(ref, { quality: "lowestvideo" }).on(
  "progress",
  (_, downloaded, total) => {
...
const ffmpegProcess = cp.spawn(
  ...
  [
    ...
    "-map",
    "1:v",
    // Re-encode video to H.264 and audio to AAC to ensure MP4 compatibility
    "-c:v",
    "libx264",
    "-c:a",
    "aac",
    // Define output file
    "out.mp4", // Ensure output is MP4
  ],
  {
    windowsHide: true,
    stdio: [
